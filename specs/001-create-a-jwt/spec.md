...
3. For each unclear aspect:
   → [NEEDS CLARIFICATION: Token expiration times for access/refresh tokens]
   → [NEEDS CLARIFICATION: Rate limiting requirements for authentication endpoints]
   → [NEEDS CLARIFICATION: Account lockout policy after failed login attempts]
   → [NEEDS CLARIFICATION: Password complexity requirements]
   → [NEEDS CLARIFICATION: What specific user identity information should be returned in validation]
   → [NEEDS CLARIFICATION: Concurrent session limits per user]
   → [NEEDS CLARIFICATION: Refresh token rotation policy]
...
- **FR-002**: System MUST validate customer credentials by calling a configurable customer validation endpoint
- **FR-003**: System MUST validate employee credentials by calling a configurable employee validation endpoint
...
- **FR-018**: System MUST verify token signature using ES256 (ECDSA P-256) asymmetric cryptography algorithm. The system MUST validate that the token algorithm matches the expected algorithm from an allowlist (ES256), and MUST reject tokens with unexpected or 'none' algorithms. EdDSA support may be added in future versions when .NET provides native support
...
- **FR-026**: System MUST lock user accounts (both customer and employee user types) for 15 minutes after 5 consecutive failed login attempts
...
- **FR-053**: System MUST implement IP-based rate limiting with a maximum of 20 failed login attempts from a single IP address across ALL accounts within any 15-minute window, resulting in temporary IP blocking
- **FR-054**: System MUST implement progressive authentication delays after failed attempts: 1 second delay after 3 attempts, 2 seconds after 4 attempts, and 4 seconds after 5 attempts, before processing the authentication request
- **FR-055**: System MUST include rate limit information in authentication response headers indicating current limit remaining before lockout, time until limit reset, and retry-after duration when limit is exceeded
- **FR-056**: System MUST return HTTP 429 (Too Many Requests) status code when rate limits are exceeded, with appropriate error details for client handling
...
- **FR-042**: System MUST support distributed tracing with correlation ID propagation. The system MUST accept correlation IDs from request headers (X-Correlation-ID, X-Request-ID), generate new correlation IDs if not provided, propagate correlation IDs to all external service calls, include correlation IDs in all log entries and error responses, and support OpenTelemetry trace context propagation
...
- **FR-049**: System MUST retry failed external validation requests with exponential backoff: 3 attempts maximum, 100ms initial delay, 2x backoff multiplier (100ms → 200ms → 400ms), 5 second total timeout
